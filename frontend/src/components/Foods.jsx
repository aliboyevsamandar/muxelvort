...
export default function Foods() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const Foods = response.data.filter(item => item.category === "foods");
        setData(Foods);
      } catch (error) {
  ...
  return (
    <div className="pt-32 pb-12 min-h-[696px] max-h-full bg-slate-800">
      <h1 className="text-4xl text-center text-white font-bold">Foods</h1>

      {loading ? (
        <div className="flex items-center justify-center mt-32 bg-gray-800">
          ...
          {data.length > 0 ? (
            data.map((item) => (
              <div
                ...
                className="w-[270px] cursor-pointer max-w-sm bg-white border border-gray-200 rounded-xl hover:shadow-gray-500 hover:shadow-[0px_0px_13px_3px] duration-700 hover:scale-[1.02] dark:bg-slate-900 dark:border-gray-700 h-[330px] flex flex-col"
              >
                <img
                ...
                <div className="px-5 pb-5 flex flex-col justify-between flex-grow">
                  <h5 className="text-2xl text-center font-semibold tracking-tight text-gray-900 dark:text-white mb-2">
                    {item.name}
                  </h5>
